<div class="container px-4 py-8" wire:init="loadAuthor">
    <div class="bg-white dark:bg-slate-900 rounded-2xl shadow-lg border border-slate-200/70 dark:border-slate-800/70 overflow-hidden mb-8">
        <div class="bg-gradient-to-r from-primary-dark to-secondary-dark text-white px-6 py-5 flex items-center justify-between">
            <div class="flex items-center gap-4">
                <div class="h-14 w-14 rounded-full bg-white/20 backdrop-blur text-xl font-bold flex items-center justify-center">
                    {{ strtoupper(mb_substr($author->name, 0, 1)) }}
                </div>
                <div>
                    <p class="text-sm uppercase tracking-wide text-white/80">লেখক</p>
                    <h1 class="text-3xl font-bold">{{ $author->name }}</h1>
                    @if($author->email)
                        <p class="text-sm text-white/80">{{ $author->email }}</p>
                    @endif
                </div>
            </div>
