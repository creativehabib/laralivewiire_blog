...
<div class="w-full space-y-6">
    ...
    <div class="rounded-2xl border border-dashed border-slate-300 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-900" data-dashboard-controls>
        ...
        <div class="mt-4 hidden rounded-xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-700 dark:bg-slate-800/60" data-dashboard-options-panel>
            ...
            <div class="grid grid-cols-1 gap-2.5 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4">
                @foreach ($dashboardWidgets as $widgetId => $widgetLabel)
                    <label class="flex cursor-pointer items-center gap-2 rounded-lg bg-white px-3 py-2 text-sm font-medium text-slate-700 shadow-sm ring-1 ring-slate-200 dark:bg-slate-900 dark:text-slate-200 dark:ring-slate-700">
                        ...
                        <span class="truncate">{{ $widgetLabel }}</span>
                    </label>
                @endforeach
            </div>
        </div>
    ...
    {{-- Main Widgets Grid --}}
    <div class="grid grid-cols-1 gap-4 lg:grid-cols-2 xl:grid-cols-3" data-dashboard-widgets>

        {{-- Stats Overview --}}
        <section class="min-w-0 rounded-2xl border border-slate-200 bg-white p-4 sm:p-5 shadow-sm dark:border-slate-700 dark:bg-slate-900 lg:col-span-2 xl:col-span-3" data-dashboard-widget="stats" draggable="true">
                    ...
                    <div class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-900">
                        <div class="flex items-start justify-between gap-2">
                            <div class="min-w-0 flex-1">
                                <p class="truncate text-sm font-medium text-slate-500 dark:text-slate-300">{{ $stat['label'] }}</p>
                                <p class="mt-2 text-3xl sm:text-4xl font-bold text-slate-900 dark:text-white">{{ $stat['value'] }}</p>
                            </div>
                            <span class="shrink-0 rounded-full px-2.5 py-0.5 text-xs font-semibold {{ $stat['badge'] }}">
                                {{ $stat['delta'] }}%
                            </span>
                        </div>
                        <p class="mt-2 truncate text-xs text-slate-500 dark:text-slate-400">{{ $stat['subtitle'] }}</p>
                    </div>
                @endforeach
            </div>
        </section>
        ...
        {{-- Visit Vs Visitor --}}
        <section class="min-w-0 overflow-hidden rounded-2xl border border-slate-200 bg-white p-4 sm:p-6 shadow-sm dark:border-slate-700 dark:bg-slate-900 lg:col-span-2 xl:col-span-3" data-dashboard-widget="visit-vs-visitor" draggable="true">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    <p class="text-sm font-semibold text-slate-700 dark:text-slate-200">{{ __('Visit Vs Visitor') }}</p>
                    <p class="text-xs sm:text-sm text-slate-500 dark:text-slate-400">{{ __('Monthly comparison overview') }}</p>
                </div>
            ...
            <div class="mt-6 w-full overflow-hidden">
                <div id="visitVsVisitorChart" class="h-80 w-full"></div>
            </div>
            <div class="mt-6 grid grid-cols-1 gap-3 sm:grid-cols-3">
                <div class="rounded-xl bg-slate-50 px-4 py-3 text-center dark:bg-slate-800/60">
                    ...
                    <p class="mt-1 text-xl sm:text-2xl font-semibold text-slate-800 dark:text-white">{{ number_format($visitVsVisitor['totals']['uniqueVisitors']) }}</p>
                </div>
                <div class="rounded-xl bg-slate-50 px-4 py-3 text-center dark:bg-slate-800/60">
                    <p class="text-xs uppercase tracking-wide text-slate-500 dark:text-slate-400">{{ __('Total Visits (This Month)') }}</p>
                    <p class="mt-1 text-2xl sm:text-3xl font-bold text-slate-800 dark:text-white">{{ number_format($visitVsVisitor['totals']['totalVisits']) }}</p>
                </div>
                <div class="rounded-xl bg-slate-50 px-4 py-3 text-center dark:bg-slate-800/60">
                    <p class="text-xs uppercase tracking-wide text-slate-500 dark:text-slate-400">{{ __('Total Visitors (This Month)') }}</p>
                    <p class="mt-1 text-2xl sm:text-3xl font-bold text-slate-800 dark:text-white">{{ number_format($visitVsVisitor['totals']['totalVisitors']) }}</p>
                </div>
            </div>
        </section>

        {{-- Visitors Reports --}}
        <section class="min-w-0 overflow-hidden rounded-2xl border border-slate-200 bg-white p-4 sm:p-6 shadow-sm dark:border-slate-700 dark:bg-slate-900 lg:col-span-2 xl:col-span-3" data-dashboard-widget="visitors-reports" draggable="true">
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div>
                    <p class="text-sm font-semibold text-slate-600 dark:text-slate-300">{{ __('Visitors Reports') }}</p>
                    <p class="text-xs sm:text-sm text-slate-500 dark:text-slate-400">{{ __('Current vs previous year performance') }}</p>
                </div>
                <div class="flex flex-wrap items-center gap-3 text-xs text-slate-500 dark:text-slate-400">
                    <span class="inline-flex items-center gap-1.5"><span class="size-2.5 rounded-full bg-indigo-500"></span>{{ $visitorSeries[0]['name'] ?? __('Current Year') }}</span>
                    <span class="inline-flex items-center gap-1.5"><span class="size-2.5 rounded-full bg-amber-400"></span>{{ $visitorSeries[1]['name'] ?? __('Previous Year') }}</span>
                </div>
            </div>
            <div class="mt-6 w-full overflow-hidden">
                <div id="visitorsChart" class="h-80 w-full"></div>
            </div>
        </section>
        ...
        @foreach ($pieCharts as $pie)
            ...
            <section class="min-w-0 rounded-2xl border border-slate-200 bg-white p-4 sm:p-5 shadow-sm dark:border-slate-700 dark:bg-slate-900" data-dashboard-widget="{{ $pieWidgetId }}" draggable="true">
                <div class="flex items-center justify-between gap-2">
                    <p class="truncate text-sm font-semibold text-slate-600 dark:text-slate-200">{{ $pie['title'] }}</p>
                    <span class="shrink-0 text-xs text-slate-500 dark:text-slate-400">{{ array_sum($pie['series']) }} {{ __('Entries') }}</span>
                </div>
                <div class="mt-4 space-y-3">
                    <div id="{{ $pie['id'] }}" class="h-56 w-full overflow-hidden"></div>
                    <div class="space-y-2">
                        @foreach ($pie['data'] as $segment)
                            <div class="flex items-center justify-between text-sm">
                                <div class="flex items-center gap-2 truncate pr-2">
                                    <span class="size-2.5 shrink-0 rounded-full {{ $segment['color'] }}"></span>
                                    <span class="truncate text-slate-600 dark:text-slate-200">{{ $segment['name'] }}</span>
                                </div>
                                <span class="shrink-0 font-medium text-slate-500 dark:text-slate-400">{{ $segment['value'] }}</span>
                            </div>
                        @endforeach
                    </div>
                </div>
        ...
        {{-- Activity Logs --}}
        <section class="min-w-0 rounded-2xl border border-slate-200 bg-white p-4 sm:p-6 shadow-sm dark:border-slate-700 dark:bg-slate-900 lg:col-span-2 xl:col-span-3" data-dashboard-widget="activity-logs" draggable="true">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm font-semibold text-slate-700 dark:text-slate-200">{{ __('Activity Logs') }}</p>
                    <p class="text-xs sm:text-sm text-slate-500 dark:text-slate-400">{{ __('Recent platform activities') }}</p>
                </div>
            ...
            <div class="mt-4 divide-y divide-slate-100 text-sm text-slate-700 dark:divide-slate-800 dark:text-slate-200">
                @forelse ($activityLogs as $log)
                    <div class="flex flex-col sm:flex-row sm:items-start justify-between gap-2 sm:gap-4 py-3">
                        <div class="flex items-start gap-3 min-w-0">
                            <span class="flex size-9 shrink-0 items-center justify-center rounded-full bg-slate-100 text-sm font-bold text-slate-700 dark:bg-slate-800 dark:text-slate-200">
                                {{ Str::upper(Str::substr($log->causer?->name ?? __('System'), 0, 1)) }}
                            </span>
                            <div class="min-w-0 space-y-0.5">
                                <p class="truncate font-semibold">{{ $log->causer?->name ?? __('System') }}</p>
                                <p class="text-xs text-slate-500 dark:text-slate-400 break-words">{{ $log->description }}</p>
                            </div>
                        </div>
                        <div class="text-left sm:text-right text-xs text-slate-500 dark:text-slate-400 pl-12 sm:pl-0">
        ...
        {{-- Post Status --}}
        <section class="min-w-0 rounded-2xl border border-slate-200 bg-white p-4 sm:p-5 shadow-sm dark:border-slate-700 dark:bg-slate-900" data-dashboard-widget="post-status" draggable="true">
            <div class="flex items-center justify-between">
            ...
            <div class="w-full overflow-hidden">
                <div id="post-status-chart" class="mt-4 h-72 w-full"></div>
            </div>
            <div class="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm text-slate-600 dark:text-slate-200">
                @foreach ($statusChart['categories'] as $index => $status)
                    <div class="flex items-center justify-between rounded-lg bg-slate-50 px-3 py-2 dark:bg-slate-800/60">
                        <div class="flex items-center gap-2 truncate pr-2">
                            <span class="size-2.5 shrink-0 rounded-full" style="background-color: {{ $statusChart['colors'][$index] ?? '#94a3b8' }}"></span>
                            <span class="truncate">{{ $status }}</span>
                        </div>
                        <span class="shrink-0 text-xs font-semibold text-slate-500 dark:text-slate-400">{{ $statusChart['series'][$index] ?? 0 }}</span>
                    </div>
                @endforeach
            </div>
        </section>
        ...
        {{-- Popular Tags --}}
        <section class="min-w-0 rounded-2xl border border-slate-200 bg-white p-4 sm:p-5 shadow-sm dark:border-slate-700 dark:bg-slate-900" data-dashboard-widget="popular-tags" draggable="true">
            <div class="flex items-center justify-between">
            ...
            <div class="mt-4 flex flex-wrap gap-2">
                @foreach ($popularTags as $tag)
                    <span class="inline-flex items-center gap-1.5 rounded-full bg-slate-100 px-3 py-1 text-xs font-medium text-slate-700 dark:bg-slate-800 dark:text-slate-200">
                        #{{ Str::slug($tag->name) }}
                        <span class="rounded-full bg-white px-1.5 py-0.5 text-[10px] font-semibold text-slate-600 shadow-sm dark:bg-slate-900 dark:text-slate-300">{{ $tag->posts_count }}</span>
                    </span>
                @endforeach
            </div>
        </section>
        ...
        {{-- Top Categories --}}
        <section class="min-w-0 rounded-2xl border border-slate-200 bg-white p-4 sm:p-5 shadow-sm dark:border-slate-700 dark:bg-slate-900" data-dashboard-widget="top-categories" draggable="true">
            <div class="flex items-center justify-between">
            ...
            <div class="mt-4 space-y-2.5 text-sm text-slate-600 dark:text-slate-200">
                @foreach ($topCategories as $category)
                    <div class="flex items-center justify-between rounded-lg border border-slate-100 px-3 py-2 dark:border-slate-800">
                        ...
                        <span class="shrink-0 font-medium text-slate-500 dark:text-slate-400">{{ $category->posts_count }}</span>
                    </div>
                @endforeach
            </div>
        </section>
        ...
        {{-- Latest Members --}}
        <section class="min-w-0 rounded-2xl border border-slate-200 bg-white p-4 sm:p-5 shadow-sm dark:border-slate-700 dark:bg-slate-900" data-dashboard-widget="latest-members" draggable="true">
            <div class="flex items-center justify-between">
                ...
                @foreach ($latestMembers as $member)
                    <div class="flex items-center justify-between py-2.5">
                        <div class="flex items-center gap-3 min-w-0">
                            <span class="flex size-9 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-emerald-500 to-blue-500 text-sm font-semibold text-white">
                                {{ Str::upper(Str::substr($member->name, 0, 1)) }}
                            </span>
                            <div class="min-w-0">
                                <p class="truncate font-semibold">{{ $member->name }}</p>
                                <p class="truncate text-xs text-slate-500 dark:text-slate-400">{{ __('Member since') }} {{ $member->created_at->format('M d, Y') }}</p>
                            </div>
                        </div>
                    </div>
                @endforeach
        ...
        {{-- Most Viewed Posts --}}
        <section class="min-w-0 rounded-2xl border border-slate-200 bg-white p-4 sm:p-5 shadow-sm dark:border-slate-700 dark:bg-slate-900" data-dashboard-widget="most-viewed-posts" draggable="true">
            <div class="flex items-center justify-between">
            ...
            <div class="mt-4 space-y-2.5 text-sm text-slate-700 dark:text-slate-200">
                @foreach ($mostViewedPosts as $post)
                    <div class="flex items-center justify-between rounded-lg border border-slate-100 px-3 py-2 dark:border-slate-800">
                        <span class="truncate pr-2">{{ $post->name }}</span>
                        <span class="shrink-0 font-medium text-slate-500 dark:text-slate-400">{{ number_format($post->views ?? 0) }}</span>
                    </div>
                @endforeach
            </div>
        </section>
        ...
        {{-- Popular Categories --}}
        <section class="min-w-0 rounded-2xl border border-slate-200 bg-white p-4 sm:p-5 shadow-sm dark:border-slate-700 dark:bg-slate-900" data-dashboard-widget="popular-categories" draggable="true">
            <div class="flex items-center justify-between">
            ...
            <div class="mt-4 space-y-2.5 text-sm text-slate-700 dark:text-slate-200">
                @foreach ($topCategories->take(5) as $category)
                    <div class="flex items-center justify-between rounded-lg border border-slate-100 px-3 py-2 dark:border-slate-800">
                        <span class="truncate pr-2">{{ $category->name }}</span>
                        <span class="shrink-0 font-medium text-slate-500 dark:text-slate-400">{{ number_format($category->posts_count) }}</span>
                    </div>
                @endforeach
            </div>
        </section>
        ...
        {{-- Latest Posts --}}
        <section class="min-w-0 rounded-2xl border border-slate-200 bg-white p-4 sm:p-5 shadow-sm dark:border-slate-700 dark:bg-slate-900 lg:col-span-2 xl:col-span-2" data-dashboard-widget="latest-posts" draggable="true">
            <div class="flex items-center justify-between">
            ...
            <div class="mt-4 divide-y divide-slate-100 text-sm text-slate-700 dark:divide-slate-800 dark:text-slate-200">
                @foreach ($recentPosts as $post)
                    <div class="flex items-center justify-between gap-3 py-2.5">
                        <div class="min-w-0">
                            <p class="truncate font-semibold">{{ $post->name }}</p>
                            <p class="text-xs text-slate-500 dark:text-slate-400">{{ $post->created_at?->format('M d, Y') }}</p>
                        </div>
                        <span class="shrink-0 text-xs text-slate-500 dark:text-slate-400">{{ number_format($post->views ?? 0) }} {{ __('views') }}</span>
                    </div>
                @endforeach
            </div>
        </section>
        ...
        {{-- Latest Pages --}}
        <section class="min-w-0 rounded-2xl border border-slate-200 bg-white p-4 sm:p-5 shadow-sm dark:border-slate-700 dark:bg-slate-900" data-dashboard-widget="latest-pages" draggable="true">
            <div class="flex items-center justify-between">
                ...
                @foreach ($recentPages as $page)
                    <div class="flex items-center justify-between py-2.5">
                        <div class="flex items-center gap-2 truncate pr-2">
                            ...
                            <span class="truncate font-semibold">{{ Str::title($page->name) }}</span>
                        </div>
                        <span class="shrink-0 text-xs text-slate-500 dark:text-slate-400">/{{ $page->slug }}</span>
                    </div>
                @endforeach
            </div>
        </section>
