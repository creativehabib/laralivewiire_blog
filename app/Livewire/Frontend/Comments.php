...
class Comments extends Component
{
    ...
    public function mount(Model $commentable, string $commentRelation = 'comments'): void
    {
        $this->bootCommentable($commentable, $commentRelation);
    }
    ...
    {
        $comment = $this->commentQuery()->approved()->findOrFail($commentId);
    ...
    protected function loadComments(): void
    {
        ...
        $this->comments = $this->commentQuery()
            ->whereNull('parent_id')
            ->approved()
    ...
    protected function applyCommentVisibilityRules(): void
    {
        ...
        if ($this->commentSettings['auto_close'] && $commentable->created_at) {
            ...
            if ($ageInDays >= $this->commentSettings['auto_close_days']) {
                $this->allowComments = false;
                $this->blockedReason = 'এই কনটেন্টের মন্তব্য করার সময়সীমা শেষ হয়েছে।';
            }
        }
    }
